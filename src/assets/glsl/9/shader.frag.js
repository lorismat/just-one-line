...
vec3 modified (vec2 st, float position, float thickness) {
  ...
  float noisyPos = noise(st * 1.) * 2.;
  float noisyInc = noise(vec2(st.y, st.y) * 2.) * 1.;
  float noisyExt = noise(vec2(st.x, st.x) * 20.) * 0.1;
  
  position += noisyPos * pow(t,1.);
  position += noisyInc * pow(t,1.);
  position += noisyExt * pow(t,1.);
...
void main () {
  ...
  vec3 modifiedLine1 = modified(st, 0.5, 0.2);
